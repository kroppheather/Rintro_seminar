...
######answer###########
matTree <- matrix(c(oak,maple),ncol=2,byrow=FALSE)
...
######answer###########
treeDF <- data.frame(height=c(oak,maple),
					species=c(rep("oak",10),rep("maple",10)))
...
#tree heights
######answer###########
mean(treeDF$height[treeDF$species=="oak"]-treeDF$height[treeDF$species=="maple"])
...
######answer###########
length(datM$Elev.m[datM$Region=="Canada"])
